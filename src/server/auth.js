...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, signOut, onAuthStateChanged, sendEmailVerification, sendPasswordResetEmail, confirmPasswordReset } from "firebase/auth";
...
const daftarWithGoogle = async () => {
    return new Promise( async (resolve, reject) => {
        try {
            ...
            const user = userCredential.user;
            const data = { uid: user.uid, email: user.email, username: user.displayName, role: "user", photoURL: user.photoURL, kelas: "", createAt: new Date() };
...
export { daftarWithManual, daftarWithGoogle, loginWithManual, Authentication, logout, resetPassword, confirmResetPassword };